...
**기본 상식**: "테스트를 통과해야만 배포한다"는 원칙은 예외 없이 지켜야 하는 규칙이 아니라 기본값입니다. 운영 장애를 급히 막아야 하는 hotfix(긴급 수정) 상황처럼 정말 예외가 필요할 수는 있습니다. 이때도 파이프라인을 몰래 우회하지 말고, "누가 승인했는지, 왜 건너뛰었는지"가 기록에 남는 절차를 미리 정해둡니다(예: 승인자가 있는 별도 hotfix 워크플로, 배포 후 반드시 테스트를 재실행하는 후속 조치).
...
Semantic Versioning(유의적 버전)이란 버전 번호를 `1.2.0`처럼 "메이저.마이너.패치" 3자리로 표기하고, 변경 성격에 따라 어느 자리를 올릴지 정해둔 규칙입니다.
